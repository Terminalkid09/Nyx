...
class ActiveDefaultAdmin2Check(BaseCheck):
    name = "active_default_admin2"
    ...
    async def run(self, base_request: dict, target_params: list[str]) -> list[CheckResult]:
        ...
        async with httpx.AsyncClient(verify=False, timeout=15) as client:
            ...
            for path in admin_paths:
                try:
                    resp = await client.post(f"{base_url}{path}", data={"username": "admin", "password": "password"}, headers=req.get("headers", {}), allow_redirects=False)
