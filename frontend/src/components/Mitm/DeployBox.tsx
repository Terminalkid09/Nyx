  ...
  return (
    <div className="p-4 rounded-lg border border-gray-800 bg-gray-900/50">
      ...
      <p className="text-xs text-gray-400 mb-3 leading-relaxed">
        ...
        target is not on the LAN.
      </p>

      {caB64 ? (
        <p className="text-xs text-green-400 mb-3">
          CA certificate embedded — the command works even with Nyx off.
        </p>
      ) : (
        <p className="text-xs text-amber-400 mb-3">
          CA certificate not loaded:{' '}
          {caMissing
            ? 'it will be downloaded at runtime (Nyx must be running).'
            : 'loading…'}
        </p>
      )}
      ...
      <p className="text-xs text-gray-500 mt-3">
        {!caB64 && !caMissing && (
          <span className="inline-flex items-center gap-1">
            ...
            Import the CA in the CA portal to embed the certificate.
          </span>
        )}
      </p>
    </div>
